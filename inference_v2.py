...
#             suppress_tokens=self.supress_tokens, 
...
#         compute_type = "int8" if device == "cpu" else "float16" # int8 or float32
...
#     transcribe_model = WhisperModel(args.whisper_model_name, args.language)
#     orig_transcript, segments = transcribe(audio_fn, transcribe_model)
#     transcribe_state = {"segments": segments}
...
def _normalize_whisper_words(segments):
    """
    Whisper segments에서 words를 정규화:
    - word 앞뒤 공백 제거
    - start/end float 보장
    - segments에 words가 없으면 명확히 실패(Whisper-only에서는 필수)
    """
    for seg in segments:
        if "words" not in seg or seg["words"] is None:
            raise RuntimeError(
                "Whisper-only 경로에서는 word-level timestamps가 필요하지만, "
                "현재 Whisper 출력에 segment['words']가 없습니다. "
                "해결: (1) whisper에서 word_timestamps=True가 실제로 지원/동작하는 버전인지 확인 "
                "또는 (2) WhisperX 경로를 사용해야 합니다."
            )
        for w in seg["words"]:
...
def transcribe(audio_path, transcribe_model):
    segments = transcribe_model.transcribe(audio_path)
    segments = _normalize_whisper_words(segments)
    state = get_transcribe_state(segments)
    return state["transcript"], state["segments"], state

def get_random_string():
    return "".join(str(uuid.uuid4()).split("-"))

def traditional_to_simplified_segments(segments):
    converter = opencc.OpenCC("t2s")
    for i in range(len(segments)):
        segments[i]["text"] = converter.convert(segments[i].get("text", ""))
        if "words" in segments[i] and segments[i]["words"] is not None:
            for j in range(len(segments[i]["words"])):
                if "word" in segments[i]["words"][j]:
                    segments[i]["words"][j]["word"] = converter.convert(segments[i]["words"][j]["word"])
    return segments

def get_mask_interval(transcribe_state, word_span):
    data = []
    for seg in transcribe_state["segments"]:
...
class WhisperOnlyModel:
    """
    openai/whisper 패키지 기반 (whisperx 사용 안 함)
    - load_model은 language 인자를 받지 않음
    - transcribe에서 language, suppress_tokens, word_timestamps를 지정
    """
    def __init__(self, model_name, language):
        from whisper import load_model
        self.model = load_model(model_name, device=device)
        ...
        # 숫자 토큰 억제(옵션): tokenizer에서 숫자만 디코딩되는 토큰을 suppress
        from whisper.tokenizer import get_tokenizer
        tokenizer = get_tokenizer(multilingual=(model_name != "base.en"), language=language)
    ...
    def transcribe(self, audio_path):
        # openai-whisper는 CPU에서 fp16 불가 -> 자동으로 fp32로 경고 후 처리
        ...
        # 숫자->단어 후처리(선택): segment 텍스트만 바꾸고 words는 그대로 둬도 됨
        for seg in segments:
            if "text" in seg and isinstance(seg["text"], str):
                seg["text"] = replace_numbers_with_words(seg["text"])
        return segments

def parse_args():
    parser = argparse.ArgumentParser(description="inference speech editing (whisper-only)")
    parser.add_argument("--sub_amount", type=float, default=0.12)
    parser.add_argument("--codec_audio_sr", type=int, default=16000)
    parser.add_argument("--codec_sr", type=int, default=50)
    parser.add_argument("--top_k", type=int, default=0)
    parser.add_argument("--top_p", type=float, default=0.8)
    ...
    parser.add_argument("--language", type=str, choices=["en", "zh"], required=True)
    parser.add_argument("--model_path", type=str, required=True)
    parser.add_argument("--codec_path", type=str, required=True)
    parser.add_argument("--orig_audio", type=str, required=True)
...
def _torch_load_compat(path):
    ...
    - 최신(PyTorch 2.6+)에서 weights_only 기본이 True로 바뀌어 UnpicklingError가 날 수 있음
    - 구버전에서는 weights_only 인자가 없을 수 있음 -> TypeError fallback
    """
    try:
        return torch.load(path, map_location="cpu", weights_only=False)
    except TypeError:
        # weights_only 인자 미지원(구버전)
...
def main(args):
    ...
    # ----- SSR model load -----
    ckpt = _torch_load_compat(os.path.join(args.model_path))
    ...
    os.system(f"cp {args.orig_audio} {args.temp_folder}")
    filename = os.path.splitext(os.path.basename(args.orig_audio))[0]
    audio_fn = f"{args.temp_folder}/{filename}.wav"
    ...
    # ===== Whisper-only path =====
    transcribe_model = WhisperOnlyModel(args.whisper_model_name, args.language)
    orig_transcript, segments, transcribe_state = transcribe(audio_fn, transcribe_model)

    if args.language == "zh":
        converter = opencc.OpenCC("t2s")
        orig_transcript = converter.convert(orig_transcript)
        segments = traditional_to_simplified_segments(segments)
    ...
    if args.tts:
        ...
        if duration > args.prompt_length:
            for seg in transcribe_state["segments"]:
                for w in seg.get("words", []):
                    if w["end"] >= args.prompt_length:
                        cut_length = min(w["end"], cut_length)
        ...
        target_transcript_copy = target_transcript
        if args.language == "en":
            target_transcript_copy = target_transcript_copy.split(" ")[0]
        else:
            target_transcript_copy = target_transcript_copy[0]
    ...
    if not args.tts:
        ...
        starting_intervals, ending_intervals = [], []
        for orig_span in orig_spans:
            start, end = get_mask_interval(transcribe_state, orig_span)
            starting_intervals.append(start)
            ending_intervals.append(end)
        ...
        morphed_span = [
            [max(s - args.sub_amount, 0), min(e + args.sub_amount, audio_dur)]
            for s, e in zip(starting_intervals, ending_intervals)
        ]
        morphed_span = combine_spans(morphed_span, threshold=0.2)
        print("morphed_spans: ", morphed_span)

        torch.save(morphed_span, f"{args.output_dir}/{args.savename}_mask.pt")
        mask_interval = torch.LongTensor(
            [[round(sp[0] * args.codec_sr), round(sp[1] * args.codec_sr)] for sp in morphed_span]
        )
    else:
        info = torchaudio.info(audio_fn)
        audio_dur = info.num_frames / info.sample_rate
        morphed_span = [(audio_dur, audio_dur)]
        mask_interval = torch.LongTensor(
            [[round(sp[0] * args.codec_sr), round(sp[1] * args.codec_sr)] for sp in morphed_span]
        )
        print("mask_interval: ", mask_interval)
    ...
    for num in tqdm(range(args.sample_batch_size)):
        ...
        if args.tts:
            # 생성 결과에서 프롬프트 앞부분 제거(Whisper-only)
            new_transcript, new_segments, new_state = transcribe(save_fn_new, transcribe_model)

            if args.language == "zh":
                new_segments = traditional_to_simplified_segments(new_segments)
                new_state = get_transcribe_state(new_segments)
                tmp1 = new_state["segments"][0]["words"][0]["word"]
                tmp2 = target_transcript_copy
            else:
                tmp1 = new_state["segments"][0]["words"][0]["word"].lower()
                tmp2 = target_transcript_copy.lower()
    ...
    shutil.copyfile(audio_fn, f"{args.output_dir}/{args.savename}_orig.wav")

    elapsed = time.time() - start_time
    print(f"Running time: {elapsed:.4f} s")
